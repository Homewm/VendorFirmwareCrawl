...
from FirmCrawler.settings import firmlist_fc, MONGO_URI,dirs_root,MONGO_DATABASE,MONGO_COLLECTION,file_size
...
def download_url(cur):
    ...
    #
    # name1 = a + name
    filename = os.path.join(dirs1, name)   # 定义文件的绝对路径
    model = '%Y-%m-%d %X'
    ...
    if os.path.exists(filename):
        ...
        if os.path.getsize(filename) > 1:
            ...
            #collection.update({'_id': cur["_id"]}, {"$set": {'Status': 0,'Path':filename}})
            collection.update({'_id': cur['_id']}, {
                "$set": {
                    ...
                    'Down_date': time.strftime(model, time.localtime())}})
            return
